/*
 * This class is intended to be the main class for this Project.
 * All necessary methods are getting calls from this class.
 *
 */
...
public class User {
    ...
    //************************************************************ Fields ************************************************************
    /* 2D Array to store admin credentials. Default credentials are stored on [0][0] index.
      * Max num of admins can be 10
     */
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\n\t\t\t\t\t\t\t To proceed further, please enter a value");
        System.out.println("\n\t\t\t\t\t------------- Default Username && Password is root-root-------------");
        System.out.println("\n\t\t------------- Using Default Credentials will restrict you to just view the list of Passengers-------------");
        displayMainMenu();
        int desiredOption = scn.nextInt();
        while (desiredOption < 0 || desiredOption > 5) {
            System.out.print("ERROR!! Please enter value between 0 - 5. Enter the value again :\t");
            desiredOption = scn.nextInt();
        }

        do {
            Scanner read1 = new Scanner(System.in);
            /* If desiredOption = 1 then call the login method
             * if default credentials are used then set the permission level to standard/default 
                    where 
                    the user can just view the customer's data. If not found, then return -1
             * if data is found then show the user display menu for adding, updating, deleting and searching users/customers
             * */
            if (desiredOption == 1) {
                ...
                if (r1.isPrivilegedUserOrNot(username, password) == -1) {  //not found
                ...
                } else if (r1.isPrivilegedUserOrNot(username, password) == 0) { //default
                    System.out.println("You've standard/default privileges to access the data.\nYou can just view customers' data and cannot perform any actions on them!");
                    ...
                    /*Display the CRUD operations to be performed by the privileged user.....Which includes Creating, Updating
                     * Searching and deleting a customer */
                    do {
                        ...
                        if (desiredOption == 1) {
                            ...
                            //If 4 is entered, then ask the user to enter the customer id, and then delete that customer
                            ...
                            System.out.println("Thanks for Using NYC Airlines Reservation System :)");

                        } else {
                            System.out.println("Invalid Choice...Looks like you're robot, entering values randomly. You've to login again.");
                            bookingAndReserving.displayArtWork(22);
                            desiredOption = 0;
                        }
                    ...
                    do {
                        ...
                        if (desiredChoice == 1) {
                            ...
                            while (numOfTickets > 10) {
                                System.out.print("ERROR!! You can't book more than 10 tickets at a time for single flight! \nPlease enter number of tickets again : ");
                                numOfTickets = scn.nextInt();
                            }
                            ...
                            bookingAndReserving.displayArtWork(3);
                            System.out.print("ARE YOU SURE YOU WANT TO DELETE THIS ACCOUNT? It's an irreversible action. Enter Y/y to confirm:  ");
    ...
    static void manualInstructions() {
        Scanner scn = new Scanner(System.in);
        System.out.printf("%n%n%50s %s Welcome to NYC Airlines User Manual %s", "", "+++++++++++++++++", "+++++++++++++++++");
